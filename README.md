...
- 관상 + 사주 + 타로를 합쳐 종합 운세 리포트 생성
- 타로 카테고리 선택 후 쓰리카드 스프레드 자동 뽑기
- 위인 매칭 가중치: 관상 30% + 사주 40% + 타로 30%
- 재물운, 연애운, 직업운, 건강운 통합 분석

### 공통

- 비회원도 분석 가능 (로그인 불필요), 비회원 분석 요청은 서비스 통계 목적으로 IP·분석유형·시각 기록
- 로그인 시 분석 이력 저장 및 조회
- Supabase pgvector 하이브리드 검색 RAG로 관상학/사주학/타로 지식 조회
...
physiognomy/
├── backend/
│   ├── main.py                  # FastAPI 엔트리포인트 (port 13351)
│   ├── middleware/
...
│   │   ├── llm.py               # OpenAI 분석 생성
│   │   ├── storage.py           # Supabase Storage (face-images 버킷, 미사용 예정)
│   │   ├── usage_log.py         # 비회원 사용 로그
│   │   └── history.py           # 이력 저장
│   └── requirements.txt
├── frontend/
│   ├── app/
│   │   ├── page.tsx             # 메인 (관상/사주/타로/종합 선택)
│   │   ├── login/               # 로그인 페이지
│   │   ├── signup/              # 회원가입 페이지
│   │   ├── face/                # 관상 분석 + 결과
│   │   ├── saju/                # 사주 분석 + 결과
...
supabase/migrations/003_anonymous_usage_logs.sql
```
...
| `SUPABASE_SERVICE_ROLE_KEY`     | Backend  | Supabase service role key (RLS 우회, 이력 저장·비회원 로그용) |
...
사주 분석. JSON body: `{ birth_year, birth_month, birth_day, birth_hour, gender, is_lunar, is_leap_month, stream }`.

### `POST /api/tarot`

타로 분석. JSON body: `{ category, stream }`. category는 "연애", "재물", "직업", "건강", "오늘의 운세" 중 택1.

### `POST /api/combined`

종합 분석 (관상+사주+타로). `multipart/form-data`로 `file` + 생년월일 정보 + `tarot_category`.
